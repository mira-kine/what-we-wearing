...
import { SLOTS, type CanvasState, type ClosetItem, type SlotKey } from '../lib/types'
...
export function Builder() {
  ...
  const { event, outfit, closet: initialCloset } = useLoaderData({ from: '/$code/builder' })
  const session = getSession()!

  const [closet, setCloset] = useState<ClosetBySlot>(initialCloset)
  const [selected, setSelected] = useState<CanvasState>(outfit?.canvas_state ?? {})
  const [upload, setUpload] = useState<{ slotKey: SlotKey; progress: UploadProgress } | null>(null)
  ...
  const handleUpload = async (slotKey: SlotKey, file: File) => {
    ...
    try {
      const item = await uploadClosetItem({
        ...
        onProgress: (p) => setUpload((cur) => (cur ? { ...cur, progress: p } : cur)),
      })
  ...
  const handleDelete = async (slotKey: SlotKey, itemId: string) => {
    try {
      ...
      setSelected((prev) => (prev[slotKey] === itemId ? { ...prev, [slotKey]: undefined } : prev))
    } catch (e) {
...
/** Remove `undefined` slot keys from canvas_state so JSON stays clean. */
  ...
  return (
    ...
    >
      <Layer>
        ...
        {mannequinImg && (
          <KonvaImage image={mannequinImg} x={0} y={0} width={STAGE_WIDTH} height={STAGE_HEIGHT} />
        )}
        {SLOT_RENDER_ORDER.map((slotKey) => {
          ...
          if (!item) return null
          return <SlotPiece key={slotKey} slotKey={slotKey} url={item.image_url} />
        })}
      </Layer>
    </Stage>
  )
...
function SlotRow(props: {
  ...
  const { label, items, selectedId, uploading, onSelect, onUpload, onDelete } = props
  ...
  const phaseLabel = useMemo(() => {
    ...
    switch (uploading.phase) {
      case 'removing': {
        const pct = uploading.ratio ? ` ${Math.round(uploading.ratio * 100)}%` : ''
            ...
            <div key={item.id} className="relative flex-shrink-0 group">
              <button
                ...
                className={
                  ...
                  (isSelected ? 'border-accent' : 'border-foreground/15 hover:border-secondary')
                }
